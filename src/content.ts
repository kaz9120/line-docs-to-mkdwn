...
function copyMarkdownToClipboard(button: HTMLButtonElement) {
    ...
    turndownService.addRule('customBlock', {
        filter: (node: any) => node.classList && node.classList.contains('custom-block'),
        replacement: function (_content: string, node: any) {
            const title = node.querySelector('.custom-block-title')?.textContent?.trim() || '';
    ...
    const contentElement = document.querySelector('.content__default')?.cloneNode(true) as HTMLElement;
    if (!contentElement) return;
    
    // ヘッダーアンカーを削除
    contentElement.querySelectorAll('a.header-anchor').forEach((el: any) => el.remove());
