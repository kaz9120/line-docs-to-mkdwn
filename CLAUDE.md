...
5. **バージョン番号の更新（必須・プッシュ前に必ず実行）**
   ```bash
   # package.jsonのversionフィールドを編集
   # セマンティックバージョニングに従って更新：
   # - 新機能追加: マイナーバージョンをインクリメント（例: 1.3.0 → 1.4.0）
   # - バグ修正・軽微な変更: パッチバージョンをインクリメント（例: 1.3.2 → 1.3.3）
   # - 破壊的変更: メジャーバージョンをインクリメント（例: 1.3.0 → 2.0.0）
   ```

   **⚠️ 重要: プッシュ前の必須チェック項目**
   - [ ] `package.json` のバージョンを更新したか？
   - [ ] バージョン番号が変更内容（新機能/バグ修正/破壊的変更）に適合しているか？

   **バージョン更新を忘れた場合:**
   - mainブランチへのマージ後、自動デプロイ時に古いバージョンで公開されてしまう
   - Chrome Web Storeで正しいバージョン管理ができなくなる

   **Claude Code への指示（開発者向け注意）:**
   - コード変更を含むプルリクエストを作成する際は、**必ずバージョン番号を更新**すること
   - バージョン更新がない場合は、プッシュ前に必ず確認すること
...
- **⚠️ 重要: プッシュ前に必ずバージョン番号を更新してください（package.json）**
...
2. **Markdownを生成**
   ...
   - 各URLに**並列で**アクセスして変換（CPUコア数に応じた並列実行）
   - `./markdown/` 配下に自動的にパスを生成して保存
   - 失敗した場合、自動的に4回までリトライ

   **注意**: 並列実行により高速化されますが、URLが多い場合は数分かかることがあります。
...
Markdown生成は並列実行とリトライで安定性を確保し、以下の設定で実行されます：

- **並列実行**: CPUコア数に応じた自動並列実行
- **タイムアウト**: テスト1件あたり60秒
- **リトライ**: 失敗時に最大4回まで自動リトライ
